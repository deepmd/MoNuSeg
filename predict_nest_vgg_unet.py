...
from models.nest_unet import Nest_VGG_UNet16
...
########################### Config Predict ##############################
net = Nest_VGG_UNet16(num_classes=4, pretrained=False).cuda()
net.eval()

weight_path = os.path.join(WEIGHTS_DIR, 'final2/unet_38_0.6597.pth')
...
do_prediction(net, output_path, TEST_IDS, patch_size=128, stride=16,
              post_processing=post_processing_watershed, visualize=False, dilation=1)
