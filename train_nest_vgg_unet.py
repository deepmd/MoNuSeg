...
########################### Config Train ##############################
net = Nest_VGG_UNet16(num_classes=4, pretrained=True).cuda()
...
print('\n---------------- Training unet ----------------')
optimizer = optim.SGD(filter(lambda p:  p.requires_grad, net.parameters()), lr=5e-3, momentum=0.9, weight_decay=0.001)
