...
# def criterion_FocallLoss(input, target, ce_w=None):
#     gamma = torch.tensor([0, 0, 1, 1], dtype=torch.float).cuda(async=True)
#     ce_w = torch.tensor(ce_w, dtype=torch.float).cuda(async=True)
#
#     scaling_factors = torch.zeros_like(input)
#     for i in range(input.shape[1]):
#         scaling_factors = torch.pow(1 - F.softmax(input, dim=1), gamma[i])
#
#     loss = F.nll_loss(scaling_factors * F.log_softmax(input, dim=1), torch.squeeze(target), weight=ce_w)
...
def criterion_CCE_SoftDice(input, target, weight=None, dice_w=None, ce_w=None):
    ...
    if weight is not None:
        loss = torch.mean(weight * F.nll_loss(F.log_softmax(input, dim=1), torch.squeeze(target),
                                              weight=ce_w, reduce=False)) + \
...
def criterion_BCE_SoftDice(input, target, dice_w=None):
    alpha = 0.5
    loss = alpha * F.binary_cross_entropy_with_logits(input, target) + \
           (1 - alpha) * dice_loss_with_logits(input, target, dice_w)
    return loss
...
def ce_dice_loss_with_logits(input, target, weight=None, dice_w=None):
    probs = F.softmax(input, dim=1)
    target = helper.make_one_hot(target, C=input.shape[1])
    C = target.shape[1]
    loss = 0
    w = [1/C]*C if dice_w is None else dice_w
    for d in range(C):
...
def ce_dice_value(input, target, dice_w=None):
    probs = F.softmax(input, dim=1)
    target = helper.make_one_hot(target, C=input.shape[1])
    C = target.shape[1]
    dice = 0
